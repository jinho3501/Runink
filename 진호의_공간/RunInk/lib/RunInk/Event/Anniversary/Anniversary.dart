...
class Anniversary extends StatefulWidget {
  ...
  @override
  State<Anniversary> createState() => _MapPageState();
}

class _MapPageState extends State<Anniversary> {
  ...
  final String _apiUrl = 'http://43.203.107.133:8000/ann';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['status'] == 'success') {
          ...
          if (jsonData['tree_route']['tree_routes'] != null) {
            for (var route in jsonData['tree_route']['tree_routes']) {
              routes.add({
                'points': route.map<LatLng>((point) =>
                    LatLng(point['lat'].toDouble(), point['lng'].toDouble())
                ).toList(),
                'distance': jsonData['distance']['tree_distance'],
                'ann': 'tree'
              });
            }
          }

          if (jsonData['korea_route']['korea_routes'] != null) {
            for (var route in jsonData['korea_route']['korea_routes']) {
              routes.add({
                'points': route.map<LatLng>((point) =>
                    LatLng(point['lat'].toDouble(), point['lng'].toDouble())
                ).toList(),
                'distance': jsonData['distance']['korea_distance'],
                'ann': 'korea'
              });
            }
          }
  ...
  Widget _buildRouteCard(String title, double distance, String duration, String address, int index) {
    ...
    final String ann = route['ann'];
    String imagePath;
    String displayText;

    if (ann == 'tree') {
      imagePath = 'assets/Ann/tree.jpg';
      displayText = '크리스마스';
    } else {
      imagePath = 'assets/Ann/korea.png';
      displayText = '광복절';
    }
