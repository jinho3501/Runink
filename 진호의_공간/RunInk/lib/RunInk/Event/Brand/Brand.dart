...
class Brand extends StatefulWidget {
  ...
  @override
  State<Brand> createState() => _MapPageState();
}

class _MapPageState extends State<Brand> {
  ...
  final String _apiUrl = 'http://43.203.107.133:8000/load_image';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['status'] == 'success') {
          ...
          // LG logo route 처리
          if (jsonData['lg_route']['lg_routes'] != null) {
            for (var route in jsonData['lg_route']['lg_routes']) {
              routes.add({
                'points': route.map<LatLng>((point) =>
                    LatLng(point['lat'].toDouble(), point['lng'].toDouble())
                ).toList(),
                'distance': jsonData['distance']['lg_distance'],
                'brand': 'LG'
              });
            }
          }
          ...
          // Apple route 처리
          if (jsonData['apple_route']['apple_routes'] != null) {
            for (var route in jsonData['apple_route']['apple_routes']) {
              routes.add({
                'points': route.map<LatLng>((point) =>
                    LatLng(point['lat'].toDouble(), point['lng'].toDouble())
                ).toList(),
                'distance': jsonData['distance']['apple_distance'],
                'brand': 'Apple'
              });
            }
          }
  ...
  Widget _buildRouteCard(String title, double distance, String duration, String address, int index) {
    ...
    final String brand = route['brand'];
    String imagePath;

    if (brand == 'LG') {
      imagePath = 'assets/Brand/LG.jpeg';
    } else {
      imagePath = 'assets/Brand/Apple.jpeg';
    }
